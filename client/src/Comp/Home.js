...
function Home() {
  ...
  useEffect(() => {
    ...
    const onof = async () =>
      await axios.get("http://localhost:5001/onof").then((response) => {
      ...
      {on === "OPEN" ? (
        <div>
          <div className="fst">
            {xx && (
              <div>
                <h1 className="sujhead">SUJATHA TOTAL DELIVERY</h1>
                <img id="imb" src={fds} alt={fds} />
                <p className="pfd">
                  We deliver food items, Meats, Medicines, Liqour, Beverages, Groceries And
          ...
          {c && (
            <div>
              <h1 className="sth">Sujatha Total Delivery</h1>
